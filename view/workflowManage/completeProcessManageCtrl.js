(function() {
    ...
    // 已完成的流程的控制器
    app.controller("completeProcessManageCtrl", function($scope, $rootScope, $uibModal, $http, $filter, fac, $log) {
        ...
        $scope.find = function(pageNo) {
            ...
            fac.getPageResult("/ovu-workflow/act/process/completed", $scope.runProcessSearch, function(data) {
                $scope.pageModel = data;
    ...
    app.controller("contractProcessRecordCtrl", function($scope, $uibModalInstance, $http, items) {
        ...
        $scope.recordFind = function() {
            $http.get("/ovu-workflow/act/controller/histoicFlowList?procInsId="+$scope.curItem.processInstanceId).success(function(data) {
                if(data.code == 200) {
    ...
    app.controller("runProcessSkipCtrl", function($scope, $uibModalInstance, $http, items) {
        ...
        // 加载
        $scope.find = function() {
            $http.get("/ovu-workflow/act/actNode/getAllUserTask?processDefinitionId="+$scope.curItem.processDefinitionId).success(function(data) {
                if(data.code == 200) {
