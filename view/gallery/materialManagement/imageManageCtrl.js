(function(){
	...
	app.controller('imageManageCtrl', function ($scope, $rootScope, $sce, $uibModal, $state, $http, $filter, fac) {
		document.title = '图片管理';
        $scope.pageModel = {};
        $scope.search = {};
        ...
        $scope.createFolder = function(item){
        	...
        	if(copy.type == undefined || !copy.type == 0){
        		copy.type = 0;
        	}
            var modal = $uibModal.open({
                animation: false,
                size: 'md',
                templateUrl: '/view/gallery/materialManagement/imageManage/modal.addEditFolder.html',
                controller: 'imageFolderEditModelCtrl',
                resolve: {
                    param: copy
                }
            });
        ...
        $scope.editFile = function(item){
            ...
            var modal = $uibModal.open({
                animation: false,
                size: 'md',
                templateUrl: '/view/gallery/materialManagement/imageManage/modal.editFile.html',
                controller: 'imageFileModifyModelCtrl',
                resolve: {
                    param: copy
                }
            });
        ...
        $scope.createFile = function(){
            var modal = $uibModal.open({
                animation: false,
                size: 'md',
                templateUrl: '/view/gallery/materialManagement/imageManage/modal.addEditFile.html',
                controller: 'imageFileEditModelCtrl',
                resolve: {
                    folder: $scope.curFolder
                }
            });
            modal.result.then(function (rerList) {
                $scope.fileFind();
            }, function () {
            	$scope.fileFind();
                console.info('Modal dismissed at: ' + new Date());
	...
	// 新增编辑Controller
    app.controller('imageFolderEditModelCtrl', function ($scope,$rootScope, $http, $uibModal, $uibModalInstance, $filter, fac, param) {
    	$scope.item = param || {};
    ...
    // 新增编辑Controller
    app.controller('imageFileModifyModelCtrl', function ($scope,$rootScope, $http, $uibModal, $uibModalInstance, $filter, fac, param) {
    	$scope.item = param || {};
    ...
    //新增，编辑文件
    app.controller('imageFileEditModelCtrl', function ($scope,$rootScope, $http, $uibModal, $uibModalInstance, $filter, fac, folder) {
    ...
    app.component('imageRetModelModal', {
