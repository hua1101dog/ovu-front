(function(){
	...
	app.controller('videoManageCtrl', function ($scope, $rootScope, $sce, $uibModal, $state, $http, $filter, fac) {
		document.title = '视频管理';
        $scope.pageModel = {};
        $scope.search = {};
        ...
        $scope.createFolder = function(item){
        	...
        	if(copy.type == undefined || !copy.type == 3){
        		copy.type = 3;
        	}
            var modal = $uibModal.open({
                animation: false,
                size: 'md',
                templateUrl: '/view/gallery/materialManagement/videoManage/modal.addEditFolder.html',
                controller: 'videoFolderEditModelCtrl',
                resolve: {
                    param: copy
                }
            });
        ...
        $scope.editFile = function(item){
            ...
            var modal = $uibModal.open({
                animation: false,
                size: 'md',
                templateUrl: '/view/gallery/materialManagement/videoManage/modal.editFile.html',
                controller: 'videoFileModifyModelCtrl',
                resolve: {
                    param: copy
                }
            });
        ...
        $scope.createFile = function(){
            var modal = $uibModal.open({
                animation: false,
                size: 'md',
                templateUrl: '/view/gallery/materialManagement/videoManage/modal.addEditFile.html',
                controller: 'videoFileEditModelCtrl',
                resolve: {
        ...
        $scope.showFile = function(o){
        	var modal = $uibModal.open({
                animation: false,
                size: 'lg',
                templateUrl: '/view/gallery/materialManagement/videoManage/modal.showFile.html',
                controller: 'videoShowFileModelCtrl',
                resolve: {
                    item: o
                }
            });
	...
	// 新增编辑Controller
    app.controller('videoFolderEditModelCtrl', function ($scope,$rootScope, $http, $uibModal, $uibModalInstance, $filter, fac, param) {
    	$scope.item = param || {};
    ...
    // 新增编辑Controller
    app.controller('videoFileModifyModelCtrl', function ($scope,$rootScope, $http, $uibModal, $uibModalInstance, $filter, fac, param) {
    	$scope.item = param || {};
    ...
    //新增，编辑文件
    app.controller('videoFileEditModelCtrl', function ($scope,$rootScope, $http, $uibModal, $uibModalInstance, $filter, fac, folder) {
    	$scope.items = [{fileName:"",filePath:""}]
    	
    	//上文件
		$scope.uploadFile = function(item){
			$rootScope.addLimitFile(item,"filePath","fileName",[".mp4"]);
		}
    ...
    //展示文件
    app.controller('videoShowFileModelCtrl', function ($scope,$rootScope, $http, $uibModal, $sce ,$uibModalInstance, $filter, fac, item) {
		$scope.video = angular.copy(item);
//		videojs.options.flash.swf = "/res/video/video-js.swf"
		$scope.video.filePath = $sce.trustAsResourceUrl($scope.video.filePath);
    ...
    app.component('videoRetModelModal', {
