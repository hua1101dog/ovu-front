(function(){
	...
	app.controller('paintingsManageCtrl', function ($scope, $rootScope, $sce, $uibModal, $state, $http, $filter, fac) {
		document.title = '画册管理';
        $scope.pageModel = {};
        $scope.search = {};
        ...
        $scope.createFolder = function(item){
        	...
        	if(copy.type == undefined || !copy.type == 1){
        		copy.type = 1;
        	}
            var modal = $uibModal.open({
                animation: false,
                size: 'md',
                templateUrl: '/view/gallery/materialManagement/paintingsManage/modal.addEditFolder.html',
                controller: 'paintingsFolderEditModelCtrl',
                resolve: {
                    param: copy
                }
            });
        ...
        $scope.editFile = function(item){
            ...
            var modal = $uibModal.open({
                animation: false,
                size: 'md',
                templateUrl: '/view/gallery/materialManagement/paintingsManage/modal.editFile.html',
                controller: 'paintingsFileModifyModelCtrl',
                resolve: {
                    param: copy
                }
            });
        ...
        $scope.createFile = function(){
            var modal = $uibModal.open({
                animation: false,
                size: 'md',
                templateUrl: '/view/gallery/materialManagement/paintingsManage/modal.addEditFile.html',
                controller: 'paintingsFileEditModelCtrl',
                resolve: {
        ...
        $scope.showFile = function(o){
        	var modal = $uibModal.open({
                animation: false,
                size: 'lg',
                templateUrl: '/view/gallery/materialManagement/paintingsManage/modal.showFile.html',
                controller: 'paintingsShowFileModelCtrl',
                resolve: {
                    item: o
                }
            });
	...
	// 新增编辑Controller
    app.controller('paintingsFolderEditModelCtrl', function ($scope,$rootScope, $http, $uibModal, $uibModalInstance, $filter, fac, param) {
    	$scope.item = param || {};
    ...
    // 新增编辑Controller
    app.controller('paintingsFileModifyModelCtrl', function ($scope,$rootScope, $http, $uibModal, $uibModalInstance, $filter, fac, param) {
    	$scope.item = param || {};
    ...
    //新增，编辑文件
    app.controller('paintingsFileEditModelCtrl', function ($scope,$rootScope, $http, $uibModal, $uibModalInstance, $filter, fac, folder) {
    	$scope.items = [{fileName:"",filePath:""}]
    	
    	//上传画册文件
		$scope.uploadFile = function(item){
			$rootScope.addLimitFile(item,"filePath","fileName",
    	...
    	$scope.openRetPage = function(retList){
            var modal = $uibModal.open({
                animation: true,
                component: 'paintingRetModelModal',
                size: 'lg',
                resolve: {
                    item: function() {
                        return retList;
    ...
    //展示文件
    app.controller('paintingsShowFileModelCtrl', function ($scope,$rootScope, $http, $uibModal, $sce ,$uibModalInstance, $filter, fac, item) {
		$scope.painting = angular.copy(item);
		$scope.painting.filePath = $sce.trustAsResourceUrl($scope.painting.filePdfPath);
    ...
    app.component('paintingRetModelModal', {
