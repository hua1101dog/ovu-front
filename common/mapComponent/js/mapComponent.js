...
(function () {
    ...
    app.component('mapParkComponent', {
        ...
        controller: ['$scope', '$rootScope', '$timeout', '$http', 'AppService', 'indoorService_mp', '$state', function ($scope, $rootScope, $timeout, $http, AppService, indoorService_mp, $state) {
            var $ctrl = this;
            //组件挂载后发事件
            ...
            $ctrl.$onInit = function () {
                ...
                $ctrl.groundClick = function (item, index, NoShowFloor) {
                    ...
                    // 广播事件
                    var floor = indoorService_mp.getFloorById(parkName, $ctrl.floorId);
                    var state = {
                ...
                //重新加载地图
                $scope.$on("reloadMap"+window.location.hash, function (e) {
            ...
            function loadMap() {
                ...
                indoorService_mp.getMapData(parkId,parkName).then(function (res) {
                   ...
                   map = new AirocovMap.Map({
                    container:document.querySelector('#mapBox'),
                    // mapList: mapList,
    ...
    app.component('mapParkComponent1', {
        templateUrl: '/common/mapComponent/component/mapContainer1.html',
        bindings: {
            mapData: '<',
            onBim: '&',
            isAddWay:'<'
        ...
        controller: ['$scope', '$rootScope', '$timeout', '$http', 'AppService', 'indoorService_mp', '$state', function ($scope, $rootScope, $timeout, $http, AppService, indoorService_mp, $state) {
            ...
            function loadMap() {
                ...
                indoorService_mp.getMapData(parkId,parkName).then(function (res) {
                    ...
                    if (mapList.length == 0) {
                        map = null;
                            document.querySelector('#mapBox1').innerHTML = '';      
                        return;
                    }
                    map = null
                    document.querySelector('#mapBox1').innerHTML = '';      
                   // 初始化地图
                   map = new AirocovMap.Map({
                    container:document.querySelector('#mapBox1'),
                    // mapList: mapList,
                    mapList: JSON.parse(JSON.stringify(mapList)),
                    themeUrl: "/common/mapComponent/json/indoorMapData/theme/fillcolor.json",
                    position: {//设置相机位置
    ...
    app.service('indoorService_mp', function ($q, $http) {
        var that = this;
