...
(function () {
    ...
    app.component('pedestrianComponent', {
      templateUrl: '/common/mapComponent/component/pedestrianContainer.html',
      bindings: {
        mapData: '<',
        onBim: '&',
        isPedestrian: '<'
      },
      controller: ['$scope', '$rootScope', '$timeout', '$http', 'AppService', 'indoorServicePed', '$state', function ($scope, $rootScope, $timeout, $http, AppService, indoorServicePed, $state) {
        var $ctrl = this;
        ...
        $ctrl.$onInit = function () {
          ...
          $ctrl.groundClick = function (item, index, NoShowFloor) {
            ...
            // 广播事件
            var floor = indoorServicePed.getFloorById(parkName, $ctrl.floorId);
            var state = {
        ...
        function loadStopMap() {
          ...
          indoorServicePed.getMapData(parkNo, parkName).then(function () {
            var mapData = indoorServicePed.getPark(parkName);
            if (mapData.name) {
            ...
            if (!map) {
              ...
              map = new AirocovMap.Map({
                container:document.querySelector('#mapPedBox'),
                // mapList: mapList,
    ...
    app.service('indoorServicePed', function ($q, $http) {
      var that = this;
      this.mapData = [];
