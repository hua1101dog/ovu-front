...
(function() {
    ...
    app.directive('playVideo', ['$window', '$timeout', '$http', function($window, $timeout, $http) {
        return {
            ...
            link: function(scope, el, attr) {
                ...
                function init() {
                    /* $http.get('/ovu-screen/system/video/live.do?equipmentId=' + scope.deviceId).success(function(data) {
    ...
    /* 自定义指令，跑马灯展示列表*/
    app.directive("slideFollowTable",function($timeout){
        return {
            restrict : 'AE',
            replace : true,
            scope : {
            ...
            link : function(scope,elem,attrs) {
                $timeout(function(){
                    ...
                    var i = 0,sh;
                    var liLength = className.find("table").length;
                    var liHeight = className.find("table").height() + parseInt(className.find("table").css('border-bottom-width'));
                    className.html(className.html() + className.html());
                    // 开启定时器
