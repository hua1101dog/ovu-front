...
(function() {
	...
	function portalCtrl($scope, $http, $state, $timeout, AppService) {
        var vm = this;
        // 将得到的数据 转化为我们想要的结构
        var provinceJson = [];// 得到的省份数
        var cityJson = {};// 每个省份对应的 城市数
        var listJson = {}; //  每个城市对应的 项目
        var MAP = [];// 最终数据
        ...
        $http.get('/ovu-screen/pcos/show/park/all.do?domainId='+domainId)
            .success(function (rep) {
                if (rep.success) {
                    rep.data && rep.data.forEach(function (park) {
                        if( provinceJson.indexOf(park.PROVINCE_CODE_)==-1){
                            provinceJson.push(park.PROVINCE_CODE_);

                            var provinceName = placesMap[park.PROVINCE_CODE_]
                            cityJson[provinceName] = [];
                        }

                        var str = placesMap[park.CITY_CODE_]
                        if(JSON.stringify(cityJson[placesMap[park.PROVINCE_CODE_]]).indexOf(JSON.stringify(str))==-1){
                            var map= [park.MAP_LNG,park.MAP_LAT];
                            var obj = {name:placesMap[park.CITY_CODE_],jsonProvince:park.PROVINCE_CODE_,jsonCity:park.CITY_CODE_,list:[],map:map}

                            cityJson[placesMap[park.PROVINCE_CODE_]].push(obj);

                            var cityName = placesMap[park.CITY_CODE_]
                            listJson[cityName] = [];
                        }

                        if(listJson.hasOwnProperty(placesMap[park.CITY_CODE_] )){
                            listJson[placesMap[park.CITY_CODE_]].push(park);
                        }

                    })

                    provinceJson && provinceJson.forEach(function (park) {
                        var provinceMap = {}
                        provinceMap.province = placesMap[park];
                        provinceMap.jsonProvince = park;
                        provinceMap.city = []

                        MAP.push(provinceMap)
                    })

                    for(var x in cityJson){
                        for(var y in cityJson[x]){
                            for(var z in listJson){
                                if(cityJson[x][y].name==z){
                                    cityJson[x][y].list = listJson[z]
                                }
                            }
                        }
                    }
                    for(var x in MAP){
                        for(var y in cityJson){
                            if(MAP[x].province == y){
                                MAP[x].city = cityJson[y]
                            }
                        }
                    }
                    //  数据
        ...
        var smallChart = echarts.init(document.getElementById('small-map'));
        var option;

        // 全国地图
        $scope.china = function (chart) {
            var data = []
            for(var i=0; i<provinceJson.length; i++){
                var obj = {name:placesMap[provinceJson[i]].substring(0,2),selected:true}
                data.push(obj)
            }

            $.get('json/map/china-province/china.json', function (yCjson) {
                ...
                option = {
                    ...
                    series: [{
                        ...
                        label: {
                            normal: {
                                // show: true,//显示省份标签
                                textStyle:{color:"#2687B7"}//省份标签字体颜色
                            },
                            emphasis: {//对应的鼠标悬浮效果
                                show: true,
                                textStyle:{color:"#2687B7"}
                            }
                        },
                        itemStyle: {
                            normal: {
                                borderWidth: 1.5,//区域边框宽度
                                borderColor:'#3BA2D9',//区域边框颜色
                                areaColor:"#202C48",//区域颜色
                                shadowColor : 'rgba(255,255,255,0.2)', //默认透明
                                shadowBlur:10,
                                shadowOffsetX: 0,
                                shadowOffsetY: 0
        ...
        $scope.smallMap = function (data,chart) {

            var map = data.province?'china':data.jsonProvince;
            var mapData = data.province?data.province:data.name;
            if(mapData==data.province){
                mapData =data.province.substring(0,2)
            }

            $.get('json/map/china-province/'+map+'.json', function (yCjson) {
                echarts.registerMap(map, yCjson);
                option = {
                    ...
                    series: [{
                        ...
                        itemStyle: {
                            normal: {
                                ...
                                borderColor:'#115674',//区域边框颜色
                                areaColor:"#1B253E",//区域颜色
                            },
                            emphasis: {
                                borderWidth: 1,
                                borderColor: '#3BA2D9',
        ...
        // 省份地图
        $scope.province = function (data,chart) {

            var itemsData = [];
            var itemsMap ={};
            data.city && data.city.forEach(function (park) {
                var item = {name:park.name,value:1}
                itemsData.push(item)
                itemsMap[park.name] = park.map;
            })

            $.get('json/map/china-province/'+data.jsonProvince+'.json', function (yCjson) {
                echarts.registerMap(data.province, yCjson);
                option = {
                    backgroundColor:'#0A0E19',
                    series: [{
                        type: 'map',
                        mapType: data.province,
                        roam: false,
                        zoom:0.8,
                        data : [],
                        label: {
                        ...
                        markPoint:{
                            ...
                            itemStyle: {
                                normal: {
                                    ...
                                    label: {
                                        ...
                                        textStyle: {
                                            ...
                                            // textBorderWidth:2,
                                            fontSize: 16,
                                            shadowColor:'rgba(208,157,24,0.3)',
                                            shadowBlur:20,
                                        },
                                ...
                                emphasis: {
                                    color:'#E7BC0E',
                                    borderColor: 'rgba(208,157,24,0.3)',
                                    borderWidth:15,
                                    label:{
                                        show: true,
                                        distance: 15,
        ...
        $scope.city = function (data) {
            $scope.cityMap = new AMap.Map('city-map', {
                resizeEnable: true,
                zoom:11,
                center: [data.map[0], data.map[1]]
            });
            $scope.cityMap.setMapStyle('amap://styles/darkblue');
            var icon = new AMap.Icon({
                image: '/show-ems/res/img/icon_map_localize.png'
            });
            var markerList = [] ;
            data.list.forEach(function (item) {
                var marker = new AMap.Marker({
                    position: [item.MAP_LNG, item.MAP_LAT],
                    map: $scope.cityMap,
                    extData: item,
                    zIndex: 1000,
        ...
        // 省份选项卡点击事件
	    $scope.changeProvince = function (provinceIndex,data) {
            // 点击选项卡变色
            $('.province,.city').css('color','#3E599E');
            $('.province').eq(provinceIndex).css('color','#F5C400');

            // 点击省份出现相应的城市
            $scope.cityDate = $scope.mapData[provinceIndex];

            $('#china-map').show();
            $('#city-map').hide();
            $scope.province(data,myChart);
            $scope.smallMap(data,smallChart);
        }

        //城市选项卡点击事件
        $scope.changeCity = function (cityIndex,data) {
            // 点击选项卡变色
            $('.city').css('color','#3E599E');
            $('.city').eq(cityIndex).css('color','#F5C400');
            for(var i=0; i< $scope.mapData.length; i++){
                for(var j=0; j< $scope.mapData[i].city.length; j++){
                    if($scope.mapData[i].city[j].name == data.name){
                        $('.province').eq(i).css('color','#F5C400');
                    }
                }
            }
        ...
        $scope.small = function () {
            $('.province,.city').css('color','#3E599E');
            if(smallChart.getOption().series[0].mapType == 'china'){
                smallChart.clear();
                $scope.china(myChart);
            }else{
                var name = smallChart.getOption().series[0].mapType;
                var data = $scope.getData(placesMap[name]);
                $('#china-map').show();
                $('#city-map').hide();
                $scope.province(data,myChart);
                $scope.smallMap(data,smallChart)
        ...
        myChart.on('click', function (params) {
          ...
          var paramsProvince = [];
            for(var i=0; i<provinceJson.length; i++){
                paramsProvince.push(placesMap[provinceJson[i]].substring(0,2))
            }
            if(paramsProvince.indexOf(params.name)!=-1){
                // 清除地图
                $('.province,.city').css('color','#3E599E');
                var data = $scope.getData(params.name+'省');
                $('#city-map').hide();
                $('#china-map').show();
                $scope.province(data,myChart);
                $scope.smallMap(data,smallChart)
            ...
            if(params.seriesType === 'scatter'){
                ...
                for(var i=0; i< $scope.mapData.length; i++){
                    for(var j=0; j< $scope.mapData[i].city.length; j++){
                        if($scope.mapData[i].city[j].name == params.name){
                            data = $scope.mapData[i].city[j];

                            $('.province,.city').css('color','#3E599E');
                            $('.province').eq(i).css('color','#F5C400');
                            $('.city').eq(j).css('color','#F5C400');
                        }
                        for(var k=0; k<$scope.mapData[i].city[j].list.length; k++){
                            if($scope.mapData[i].city[j].list[k].PARK_NAME==params.name){
                                list = $scope.mapData[i].city[j].list[k];
                            }
                        }
                    }
                }
        ...
        $scope.getData = function (name) {
            for(var i=0; i< $scope.mapData.length; i++){
                if($scope.mapData[i].province == name){
                    var data = $scope.mapData[i];
                    $('.province').eq(i).css('color','#F5C400');
                }
            }
            return data
