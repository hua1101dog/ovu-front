        ...
        $httpProvider.interceptors.push(function($q, $injector, $window, $rootScope, $timeout) {
            ...
            return {
                ...
                'response': function(response) {
                    if(response.config.url.indexOf("/pcos/show") != -1 && response.data.data){
                       angular.isArray(response.data.data) && response.data.data.forEach(function (da) {
                            if(da.name){
                                da.name = da.name.replace("丽岛物业丽岛·2046",'');
                                da.name = da.name.replace("丽岛物业丽岛2046",'');
                                da.name = da.name.replace("丽岛2046",'');
                                da.name = da.name.replace("丽岛·2046",'');
                            }
                            if(da.app_name){
                                da.app_name = da.app_name.replace("丽岛物业丽岛·2046",'');
                                da.app_name = da.app_name.replace("丽岛物业丽岛2046",'');
                                da.app_name = da.app_name.replace("丽岛2046",'');
                                da.app_name = da.app_name.replace("丽岛·2046",'');
                            }
                        })
                    }
                    layer.close(layerIndex);
