...
(function() {
	...
	app.controller('takingReportCtl', function ($scope,$rootScope, $http,$filter,$uibModal,fac) {
        document.title ="承接报告";
        $scope.search={};
		$scope.pageModel={};
		app.modulePromiss.then(function(){
			...
			//var data=[{"name":"相符","value":10},{"name":"不相符","value":20}];
			//var data2=[{"name":"无问题","value":30},{"name":"有问题","value":50}];
			//var data3=[{"name":"无问题","value":20},{"name":"有问题","value":60}];
			//loadChart('stockChart','盘点清单',data);
			//loadChart('watchChart','查验清单',data2);
			//loadChart('checkChart','检测清单',data3);
		});
		...
		$scope.showEditModal = function(item){
			var modal = $uibModal.open({
				animation: false,
				size:'',
				templateUrl: 'undertaking/result/modal.takingReport.html',
				controller: 'takingReportModalCtrl'
		...
		function loadData(projectId,way,typeId){
			$http.post("/ovu-pcos/taking/init/report.do",{projectId:projectId,way:way,typeId:typeId},fac.postConfig).success(function (data) {
				if(way==1){
					loadChart('stockChart','盘点清单',data);
				}else if(way==2){
	...
	app.controller('takingReportModalCtrl', function($scope,$rootScope,$http,$uibModalInstance,$uibModal,$filter,fac,item) {
		$scope.item = item || {};
        item.result='';
        if(item.has_pass){
            if(item.has_pass==1){
