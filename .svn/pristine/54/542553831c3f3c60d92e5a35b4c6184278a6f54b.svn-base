...
(function () {
    ...
    app.component('bigScreen', {
        transclude: true,
        templateUrl: '../show-lidohy/page/component/bigScreen.html',
        bindings: {
            gaodeData: '<',
            indoorData: '<',
            parkNo: '<',
    ...
    app.service('gaodeMapService', ['$timeout', 'iconService', 'gaodeHttpServer', '$q', 'AppService', '$compile', function ($timeout, iconService, httpHelper, $q, AppService, $compile) {
        ...
        this.renderGaode = function (map, gaodeData, $ctrl, type) {
            ...
            var url = "/ovu-pcos/show-ems/res/img/2046map.png";
    ...
    app.component('indoorMap', {
        templateUrl: '../show-lidohy/page/component/indoorMap.html',
        bindings: {
            onBackToGaode: '&',
            indoorData: '<'
        },
        ...
        controller: ['$scope', '$rootScope', 'gaodeHttpServer', '$timeout', '$http', 'AppService', '$state', '$q', function ($scope, $rootScope, httpHelper, $timeout, $http, AppService, $state, $q) {
            var $ctrl = this;
            var imgBaseUrl = '/ovu-pcos/show-ems/res/img/indoor-icon/';
            ...
            $ctrl.$onInit = function () {
                ...
                var map = new AirocovMap.Map({
                    ...
                    mapList: [{ //传入本楼栋内所有楼层信息
                        name: "F1",
                        mapUrl: "/ovu-pcos/show-ems/res/indoor-data/lidaoPark.geojson" //地图文件
                        // other: dataOther //第三方数据
                    }],
                    themeUrl: "/ovu-pcos/show-ems/res/img/indoor-icon/fillcolor.json", //共用渲染文件
                ...
                function drawArrow(arrowPoints, mapCenter) {
                  ...
                  var testPoint = [[-5,26],[38,-52]];
                  var polyline = new AirocovMap.Polyline();
                  var config={
                    lineColor: '#f19b16',//线的颜色，默认颜色为'#f19b16'
                    height: 10 //围栏距地板高度，默认高度为10
                  };
                  var fenceId = polyline.addLine(arrowPoints, config);
                }
                ...
                var mapEquipment = function () {
                    ...
                    httpHelper.api('GET')('/equip/list')(params).then(function (res) {
                        ...
                        res.data.data.forEach(function (value) {
                            ...
                            if (value.latitude_ && value.longitude_) {
                                // var lat = Number.parseFloat(value.latitude_) + 0.00265842186953;
                                var lat = Number.parseFloat(value.latitude_);
                                var lon = Number.parseFloat(value.longitude_);
                                // var lon = Number.parseFloat(value.longitude_) - 0.0058591393113;
                                position = {
                                    type: "Position",
                                    sType: type,
    ...
    app.component('infoPanel', {
        transclude: true,
        templateUrl: '../show-lidohy/page/component/infoPanel.html',
        bindings: {},
        controller: function () {
            var $ctrl = this;
            $ctrl.$onInit = function () {};
    ...
    app.component('palyModal', {
        ...
        templateUrl: '/ovu-pcos/show-ems/page/component/vedio.html',
        controllerAs: 'vm',
