        ...
        if (uploadImgServer && typeof uploadImgServer === 'string') {
            ...
            xhr.onreadystatechange = function () {
                ...
                if (xhr.readyState === 4) {
                    ...
                    // wjlong begin 如果服务器 返回{success: true,...} 也算上传成功
                    // if (!hooks.customInsert && result.errno != '0') {
                    if (!hooks.customInsert && result.errno != '0' && !result.success) {
                        // wjlong end
                        // hook - fail
                        if (hooks.fail && typeof hooks.fail === 'function') {
                            hooks.fail(xhr, editor, result);
                        ...
                        //wjlong begin 如果服务器返回的是{success: true,list:[],urls:'1.png,2.png'...}
                        if (result.success && result.urls) {
                            result.data = result.urls.split(',').map(function (url) {
                                return '/ovu-base/' + url;
                            });
                        }
                        //wjlong end
