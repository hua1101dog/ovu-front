...
(function () {
    ...
    app.component('bigScreen', {
        transclude: true,
        templateUrl: '../show-ems/page/component/bigScreen.html',
        bindings: {
            gaodeData: '<',
            indoorData: '<',
            parkNo: '<',
    ...
    app.component('mapArea', {
        templateUrl: '../show-ems/page/component/mapArea.html',
        bindings: {
            gaodeData: '<',
            indoorData: '<'
        },
    ...
    app.component('gaodeMap', {
        templateUrl: '../show-ems/page/component/gaodeMap.html',
        bindings: {
            onIconClick: '&',
            gaodeData: '<'
        },
    ...
    app.service('gaodeMapService', ['$timeout', 'iconService', 'gaodeHttpServer', '$q', 'AppService', '$compile',function ($timeout, iconService, httpHelper, $q, AppService, $compile) {
        ...
        this.renderGaode = function (map, gaodeData, $ctrl, type) {
            ...
            var url = "/show-ems/res/img/2046map.png";
    ...
    app.component('indoorMap', {
        templateUrl: '../show-ems/page/component/indoorMap.html',
        bindings: {
            onBackToGaode: '&',
            indoorData: '<'
        },
        ...
        controller: ['$scope', '$rootScope', 'gaodeHttpServer', '$timeout', '$http', 'AppService', '$state', '$q', function ($scope, $rootScope, httpHelper, $timeout, $http, AppService, $state, $q) {
            ...
            $ctrl.$onInit = function () {
                /* 室内地图start */
                // 室内地图初始化
                console.log(document.getElementById("map-indoor").style.width="100%");
                  ...
                  obj.forEach(function (value,index) {
                    if (value.map_lat && value.map_lng) {
                      ...
                      position.mapTitle = {
                        ...
                        titleName: "人员姓名",
                        titleId: "人员部门",
                        param:[{name:"定位时段",value:value.position_time}]
                      };
                      dataOther.push(position);
                      arrowPoints.push(AirocovMap.Utils.GPS.gcj_decrypt_exact(value.map_lat, value.map_lng));
                      // arrowPoints.push(value.map_lat, value.map_lng);
                    }
                  });
                  mapRender(dataOther);
                  drawArrow(arrowPoints, mapCenter);
                });

                var resultArr = [];

                function drawArrow(arrowPoints, mapCenter) {
                  if(resultArr){
                    resultArr = [];
                  }
                  mapCenter = coordinatesToMercato(mapCenter);
                  arrowPoints = coordinatesToCoordinates3(mapCenter,arrowPoints);
                  arrowPoints[0] instanceof Array && arrowPoints.forEach(function (v, i) {
                    v[0] = v[0] + 127.0571680124848;
                    // v[1] = v[1];
                    v[1] = v[1] - 41.64037302322686;
                  });
                  console.log(arrowPoints);
                  resultArr = arrowPoints;
                  var testPoint = [[-122,113],[62,139],[126,-5],[-108,-93],[-122,113]];
                  // var testPoint = [];
                  // console.log(testPoint);
                  // var polyline = new AirocovMap.Polyline();
                  var polyline = new AirocovMap.Polyline();
                  var config={
                    lineColor: '#f19b16',//线的颜色，默认颜色为'#f19b16'
                    height: 0.2, //围栏距地板高度，默认高度为10
                    lineType:true
                  };
                  // var config = {
                  //   //线段半径
                  //   radius: "1",
                  //   //自定义贴图
                  //   imgUrl: "/show-ems/res/img/indoor-icon/arrow1.png",
                  //   //设置动画速度(0 <= speed <= 10)
                  //   speed: "0",
                  //   //高度
                  //   height: 2
                  ...
                  var fenceId = polyline.addLine(resultArr, config);
                  // $scope.arrowPoints = arrowPoints;
                  // var fenceId = polyline.addPath(testPoint, config);
                }

                /* 坐标转换，地图中心点暂时写死*/
                var mapCenterSrc = [114.32603219669,30.52084174836];
                // var mapCenter = [mapCenterSrc[0] + 0.00025842186953, mapCenterSrc[1] - 0.0003591393113];
                var mapCenter = mapCenterSrc;
                                            ...
                                            if (value.equip_type == "gate") {
                                                ...
                                                if (value.equip_status == 1) {
                                                  if (value.sensor_status && value.sensor_status!=1){
                                                    ...
                                                    position.logo = imgBaseUrl + "icon-fire-_null.png";
                                                }
                ...
                // 地图渲染主方法
                function mapRender(dataOther,monitor) {
                    map.resetRender({
                        mapList: [{ //传入本楼栋内所有楼层信息
                            name: "F1",
                            mapUrl: "/show-ems/res/indoor-data/lidao2046.geojson", //地图文件
                    ...
                    if(monitor){
                        var Vidicone = new AirocovMap.Vidicone();
                        var config = {
                          //长度
                          focalLength: 25,
                          //偏转角度
                        ...
                        //监控器坐标[x, y, z]
                        // debugger
                        var position = [10.9841646369638,0.2,20.513477407740574];
                        var testPt = [];
                        var tempPt = [];
                        var imc = coordinatesToMercato(mapCenter);

                        //arrowPoints.push(AirocovMap.Utils.GPS.gcj_decrypt_exact(value.map_lat, value.map_lng));

                        dataOther && dataOther.forEach(function (da) {
                            if(da.info.equip_type && da.info.equip_type ==="camera"){
                                testPt.push(da.dinates);
                                // tempPt = AirocovMap.Utils.GPS.gcj_decrypt_exact(da.dinates[1], da.dinates[0]);
                                // tempPt = coordinatesToCoordinates3(imc,da.dinates);
                                tempPt = coordinatesToCoordinates3(imc,[114.32527686068872,30.521574421869534]);
                                position[0] = tempPt[0];
                                position[1] = 1;
                                position[2] = tempPt[1];
                                // tempPt.splice(1,0,0.2);
                                // console.log(tempPt);
                                // position.push(tempPt);
                                Vidicone.drawMonitor(config, position);
                            }
                        });
                        // console.log(position);
                        console.log(testPt);
                ...
                if (type == "dispatch") {
                    ...
                    mapEquipment();
                } else if(type == "gate" || type == "fire" || type == "alarm"){//数据不仅包含本身的设备列表还需要摄像头的数据
                    mapEquip2Camera(flag);
    ...
    app.component('infoPanel', {
        transclude: true,
        templateUrl: '../show-ems/page/component/infoPanel.html',
        bindings: {},
        controller: function () {
            var $ctrl = this;
            $ctrl.$onInit = function () {};
    ...
    app.component('palyModal', {
        ...
        templateUrl: '/show-ems/page/component/vedio.html',
        controllerAs: 'vm',
