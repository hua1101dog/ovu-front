...
.factory('$uibModalStack', ['$animate', '$animateCss', '$document',
    ...
    function($animate, $animateCss, $document, $compile, $rootScope, $q, $$multiMap, $$stackedMap, $uibPosition) {
        ...
        $modalStack.modalRendered = function(modalInstance) {
            /*模态框初始化时，当点击input时敲回车键，引发的自动触发模态框第一个按钮的点击事件的解决方式
             * modify by Zn
             * */
            // begin
            // wjlong 临时修改 屏蔽掉new-todo类 防止影响装修管理巡检验收标准中的 回车换行操作 20180129
            angular.element('input').not('.new-todo').keydown(function(e) {
                if (e.keyCode == 13) {
                    angular.element(this).blur();
                }
            });
            // wjlong 模态框里 my97 datepicker 不随页面滚动 bug: 813  这里是第二部分 第一部分在datepicker（../datepicker97/WdatePicker.js）源码里
            document.querySelector('[uib-modal-window][index="0"]').onscroll = function() {
                        ...
                        function hideTooltipBind() {
                            ...
                            if (ttScope.popupCloseDelay) {
                                if (!hideTimeout) {
                                    ...
                                    if($('.showRecord')[0]){
                                        var scrollH=$('.showRecord')[0].scrollHeight
                                        var clientH=$('.showRecord')[0].clientHeight
                                        ...
                                        if(scrollH>clientH){
                                            ...
                                            $(".showRecord").bind("click",function(){
                                              ...
                                              $(".showRecord").bind("mouseleave",function(){
