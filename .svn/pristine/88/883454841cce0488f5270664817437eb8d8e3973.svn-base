...
var clickModels = require("../control/draw").clickModels;
var highLightModel = require("../control/draw").highLightModel;
var positionInfoWindows = require("../control/draw").positionInfoWindows;
var evt = require("../models/event");
THREE.OrbitControls = function(object, domElement) {
    ...
    var firstTime = 0;
    var lastTime = 0;
    //点击事件要进行的工作类型,数值从零一直增长，在click函数中编写相应的代码
    this.clickType = 0;
    //
    // public methods
    //
    ...
    function click(event) {
        if (flag) {
            var top = AirocovMap.Utils.getOffsetTop(scope.domElement);
            ...
            var intersects = raycaster.intersectObjects(scope.clickModels, true);
            var type = "clickMap";
            if (intersects[0]) {
                var obj = {};
                ...
                if (500 <= intersects[0].object.info.sType && intersects[0].object.info.sType <= 999) {
                    obj.intersect = intersects[0];
                    obj.other = intersects[0].object.info;
                    obj.id=intersects[0].object.info.id;
                    type = "clickEq";
                    x = intersects[0].object.position.x;
                    y = intersects[0].object.position.y;
                    z = intersects[0].object.position.z;

                } else if (1000 <= intersects[0].object.info.sType) {
                    obj.intersect = intersects[0].object;
                    obj.intersectInfo = intersects[0].object.info;
                    type = "clickModel";
                }
                evt.emit('click', {
                    type: type,
