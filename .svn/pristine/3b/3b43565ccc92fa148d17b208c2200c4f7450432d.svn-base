...
(function () {
    ...
    app.controller("singleDailyCtrl.", function ($scope, $rootScope, $http, $filter, $uibModal, fac) {
        document.title = "工作日报";
        $scope.pageModel = {};
        $scope.search = {};
        $scope.id = "";
        ...
        $scope.showAddModal = function (id, createTime, parkId, show) {
            ...
            var modal = $uibModal.open({
                animation: false,
                templateUrl: '../view/workreport/daily/modal.singledailyEdit.html',
                size: 'max',
                controller: 'newdailyEditModalCtrl',
                controllerAs: 'vm',
                resolve: {
                    data: {
                        id: id,
    ...
    app.controller("newdailyEditModalCtrl", function ($scope, $uibModal, $uibModalInstance, $http, fac, data) {
        ...
        vm.deptTotal = function (id,deptName) {
            vm.workrRecordList = '';
            $http.post("/ovu-pcos/pcos/workUnit/deptLog.do", { deptId: id, reportType: 1, parkId: data.parkId},fac.postConfig).success(function (res) {
