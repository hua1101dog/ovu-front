(function () {
    ...
    app.controller('inspectionRoomCtl', function ($scope, $rootScope, $http, $filter, $uibModal, fac) {
        document.title = "OVU-商业巡检";
        ...
        $scope.find = function (pageNo) {
            ...
            $scope.search.inspectionType = 1;//商业
        ...
        $scope.showNews = function (item) {
            ...
            var modal = $uibModal.open({
                animation: false,
                size: 'lg',
                templateUrl: '/view/operationManage/inspection/modal.addInspectionRoom.html',
        ...
        $scope.showDetail = function (item) {
            ...
            var modal = $uibModal.open({
                animation: false,
                size: 'lg',
                templateUrl: '/view/operationManage/inspection/modal.inspectionDetail.html',
                controller: 'details'
                , resolve: {item: copy}
            });
        }
    ...
    app.controller('details', function ($scope, $http, $uibModalInstance, $filter, fac, item) {
        $scope.item = item;

        if ($scope.item.businessType === 1) {
            $scope.item.businessTypeName = "餐饮";
        }else if ($scope.item.businessType === 2) {
            $scope.item.businessTypeName = "商业";
        }
...
//商业巡检查看
  ...
  $scope.getLeaseMonth = function (rentStartTime, rentEndTime) {
    ...
    app.controller('editInspectionCtrl', function ($scope, $http, $uibModalInstance,$uibModal,$filter, fac, item) {
        $scope.count = 1;
        $scope.item = item;
        $scope.rentStartTime=$scope.item.rentStartTime;
        $scope.rentEndTime=$scope.item.rentEndTime;
        $scope.build = {};
        $scope.item.pics = [];
      ...
      $scope.getLeaseMonth = function (rentStartTime, rentEndTime) {
        ...
        $scope.addValue = function (houseId) {
            ...
            var params = {"houseId": houseId,"inspectionType":1}
     ...
     // 选择入驻方
     app.controller('selResidentCtrl', function ($scope, $rootScope, $http, $uibModalInstance, $uibModal,$filter, fac) {
        $scope.search = {};
