(function () {
    ...
    app.controller('adjustStandardPriceCtrl', function ($scope, $rootScope, $http, $filter, $uibModal, fac) {
        document.title = "标准价调价方案查询";
        var width = $(window).width() - 300
        $('#table_stan').width(width)
        ...
        $scope.find = function (pageNo) {
            ...
            fac.getPageResult("/ovu-park/backstage/sale/saleStandardAdjustmentProject/list", $scope.search, function (data) {
                $scope.pageModel = data;
            });
        };
        //编辑／新增模态窗口
        $scope.addStandard = function (id) {

            var modal = $uibModal.open({
                animation: false,
                size: 'lg',
                templateUrl: '/view/investmentSystem/projectPrepare/standardPrice/modal.addStandardPrice.html',
                controller: 'addStandardPriceCtrl',
                resolve: {}
            });
        ...
        $scope.delReserce = function (item) {
            ...
            confirm("确认删除?", function () {
                $http.post("/ovu-park/backstage/sale/saleStandardAdjustmentProject/delete", {
                    "id": item.id
        ...
        $scope.showModal = function (id, states, isShow) {
            ...
            var modal = $uibModal.open({
                animation: false,
                size: 'lg',
                templateUrl: '/view/investmentSystem/projectPrepare/standardPrice/modal.standardMethod.html',
                controller: 'standardMethodCtrl',
                resolve: {
                    param: {
                        'id': id,
                        states: states,
        ...
        $scope.approve = function (item) {
            ...
            var modal = $uibModal.open({
                ...
                controller: 'standardApprovalCtrl',
                resolve: {
                    paramdata: {
                        id: item.id,
    ...
    //新增调价方案
    app.controller('addStandardPriceCtrl', function ($scope, $rootScope, $http, $uibModalInstance, $uibModal, $filter, fac) {
        $scope.curIndex = 1;
        ...
        $scope.save = function (form) {
            ...
            if ($scope.priceInfo.computingMethod == 1) {
                ...
                // if (!$scope.priceInfo.houseTotalPrice) {
                //     alert("请输入总价！")
                //     return
                // }
                ...
                confirm("设置的总价为" + $scope.priceInfo.houseTotalPrice + "万元，是否保存?", function () {
                    ...
                    $http.post("/ovu-park/backstage/sale/saleStandardAdjustmentProject/add", $scope.priceInfo, fac.postConfig).success(function (resp) {
                        if (resp.code == 0) {
                            window.msg("新增成功！")
                ...
                // if (!/^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/.test($scope.priceInfo.unitPrice)){
                //     alert("请输入正确的基点单价！")
                //     return
                // }
                // if (!$scope.priceInfo.unitPrice) {
                //     alert("请输入基点单价！")
                //     return
                // }
                if (!/^(([1-9][0-9]{0,8})|(([0]\.\d{1,2}|[1-9][0-9]{0,8}\.\d{1,2})))$/.test($scope.priceInfo.unitPrice)) {
        ...
        $scope.openHouseModal = function () {
            ...
            var modal = $uibModal.open({
                ...
                controller: 'staSelectRoomModalCtrl',
                resolve: {
                    houses: function () {
    ...
    //查看调价方案
    app.controller('standardMethodCtrl', function ($scope, $rootScope, $http, $uibModalInstance, $uibModal, $filter, fac, param) {
        $scope.standardInfo = {
        ...
        var initDetail = function (adjustProjectId) {
            $http.post("/ovu-park/backstage/sale/saleStandardAdjustmentProject/getDetailsById", {
                id: adjustProjectId
            }, fac.postConfig).success(function (resp) {
                if (resp.code == 0) {
                    $.extend($scope.standardInfo, resp.data);
                    $scope.isEdit = $scope.isShow && ($scope.standardInfo.approveStatus == 0 || $scope.standardInfo.approveStatus == 3) ? true : false
                    var unitPrice = $scope.standardInfo.pricingmanner == 1 ? $scope.standardInfo.standardAreaUnitPrice : $scope.standardInfo.standardAreaSuUnitPrice
                    $scope.standardInfo.unitPrice = parseFloat((unitPrice).toFixed(2))
                    $scope.totalPrice = parseFloat(($scope.standardInfo.houseTotalPrice / 10000).toFixed(2));
                    $scope.saleStatusStr = $filter("approveStatus")($scope.standardInfo.approveStatus)
                } else {
                    alert(resp.msg);
                }
            });
        ...
        $scope.computePrice = function () {
            if ($scope.standardInfo.computingMethod == 2 && $scope.standardInfo.unitPrice) {
                if ($scope.standardInfo.pricingmanner == 1) { //乘以建筑面积
                    $scope.totalPrice = parseFloat(($scope.standardInfo.unitPrice * $scope.standardInfo.areaCount / 10000).toFixed(6))
                } else { //乘以套内面积
                    $scope.totalPrice = parseFloat(($scope.standardInfo.unitPrice * $scope.standardInfo.areaSuCount / 10000).toFixed(6))
                }
            } else if ($scope.standardInfo.computingMethod == 1 && $scope.totalPrice) {
                if ($scope.standardInfo.pricingmanner == 1) { //除建筑面积
                    $scope.standardInfo.unitPrice = parseFloat(($scope.totalPrice * 10000 / $scope.standardInfo.areaCount).toFixed(2))
                } else { //除套内面积
                    $scope.standardInfo.unitPrice = parseFloat(($scope.totalPrice * 10000 / $scope.standardInfo.areaSuCount).toFixed(2))
                }
            }else{
                $scope.totalPrice = ''
                $scope.standardInfo.unitPrice = ''
            }
        }
        ...
        $scope.save = function (form) {
            ...
            if ($scope.standardInfo.computingMethod == 1) {
                if (!$scope.totalPrice) {
                    alert("请输入总价！")
                    return
                }
                ...
                confirm("设置的总价为" + $scope.totalPrice + "万元，是否保存?", function () {
                    $scope.standardInfo.houseTotalPrice = $scope.totalPrice * 10000
                    $scope.standardInfo.modifyBy = $rootScope.user.id
                    $http.post("/ovu-park/backstage/sale/saleStandardAdjustmentProject/update", $scope.standardInfo, fac.postConfig).success(function (resp) {
                        if (resp.code == 0) {
                            window.msg("修改成功！")
                            $uibModalInstance.close();
                        } else {
                ...
                if (!$scope.standardInfo.unitPrice) {
                    alert("请输入基点单价！")
                    return
                }
                
                if (!/^(([1-9][0-9]{0,8})|(([0]\.\d{1,2}|[1-9][0-9]{0,8}\.\d{1,2})))$/.test($scope.standardInfo.unitPrice)) {
                    alert("基点单价最大支持输入9位，请输入正确的基点单价！")
                    return
                }

                confirm("设置的基点单价为" + $scope.standardInfo.unitPrice + "元，是否保存?", function () {
                    $scope.standardInfo.modifyBy = $rootScope.user.id
                    $http.post("/ovu-park/backstage/sale/saleStandardAdjustmentProject/update", $scope.standardInfo, fac.postConfig).success(function (resp) {
                        if (resp.code == 0) {
                            window.msg("修改成功！")
        ...
        $scope.approval = function () {
            $http.post("/ovu-park//backstage/sale/saleStandardAdjustmentProject/startApprove", {
    ...
    // 选择房间
    app.controller('staSelectRoomModalCtrl', function ($scope, $rootScope, $http, $uibModalInstance, $filter, fac, houses) {
    ...
    //审批
    app.controller('standardApprovalCtrl', function ($scope, $rootScope, $http, $uibModalInstance, $filter, fac, paramdata) {
        $scope.approvalInfo = {
        ...
        $scope.dealApproval = function (form, status) {
            ...
            $http.post("/ovu-park/backstage/sale/saleStandardAdjustmentProject/approve", $scope.approvalInfo, fac.postConfig).success(function (resp) {
                if (resp.code == 0) {
                    window.msg("审核成功！");
                } else {
                    alert(resp.msg);
