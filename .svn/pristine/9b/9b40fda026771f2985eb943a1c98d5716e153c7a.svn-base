    ...
    .controller('AppCtrl', ['$scope', '$rootScope', '$window', '$http', '$state', 'AppService', 'MENULIST',
        function($scope, $rootScope, $window, $http, $state, AppService, MENULIST) {
            ...
            $scope.$on('$stateChangeStart', function(event, state) {
                if (state.name.indexOf('app.facility') != -1) {
                    index = 0;
                } else if (state.name.indexOf('app.company') != -1) {
                    index = 1;
            ...
            if (!$rootScope.parkLevel) {
                $rootScope.parkLevel = AppService.park && AppService.park.level;
            }
...
.service('AppService', function($http) {
    ...
    this.park = session || {};
    //项目编号
    this.parkNo = session && session.parkNo || '';
    this.floorNo = '03401110001HFJRYQ01001';
    //公共饼图
    ...
    this.getTotal = function(param) {
        return $http.get("/ovu-screen-fake/pcos/show/total", { params: param }).then(function(resp) {
            return resp.data;
        });
    };
    //获取右侧部分图表数据
    this.getChartData = function(param) {
        return $http.get("/ovu-screen-fake/pcos/show/chartData", { params: param }).then(function(resp) {
            return resp.data;
        });
    };
    //获取右侧设备
    this.getEquipList = function(param) {
        return $http.get("/ovu-screen-fake/pcos/show/equip/list", { params: param }).then(function(resp) {
            return resp.data;
        });
    };
...
.filter('toInteger',function() {
    return function(input) {
        ...
        var reservedDigit = 1;
        if(input >= 10000) {
            reservedDigit = (input % 10000 === 0) ? 0 : x;
            input = (input/10000).toFixed(reservedDigit);
        }else if(input >=0){
